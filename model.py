...
class DownConv(ME.MinkowskiNetwork):
    def __init__(self, in_channels, out_channels, D):
        ...
        self.conv = ME.MinkowskiConvolution(in_channels, out_channels, kernel_size=3, stride=2, dimension=D)
        self.norm = ME.MinkowskiBatchNorm(out_channels)
...
class Net(ME.MinkowskiNetwork):
    def __init__(self, in_channels, out_channels, D, alpha=0.5):
        ...
        self.dec4 = DecoderBox(in_channels * self.ch[4] + in_channels * self.ch[3], in_channels * self.ch[3], D, alpha, 8)
        self.dec3 = DecoderBox(in_channels * self.ch[3] + in_channels * self.ch[2], in_channels * self.ch[2], D, alpha, 4)
        self.dec2 = DecoderBox(in_channels * self.ch[2] + in_channels * self.ch[1], in_channels * self.ch[1], D, alpha, 2)
